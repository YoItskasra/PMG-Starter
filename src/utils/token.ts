 ...
 */
export function generateClientToken(boardId: string, clientId: string): string {
...
export function verifyClientToken(token: string): ClientTokenPayload | null {
  try {
    ...
    const payload: ClientTokenPayload = JSON.parse(payloadJson);

    // Check expiration (30 days)
    const now = Math.floor(Date.now() / 1000);
    if (now - payload.issuedAt > 30 * 24 * 60 * 60) return null;
